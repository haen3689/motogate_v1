...
/// Shows the BCEL OnePay QR / one-click-pay deeplink for a pending RoadTax
/// payment, and polls the payment status until BCEL confirms it (or it
/// expires). The road tax itself only becomes "paid" server-side once the
/// PubNub callback arrives — this screen is just watching for that.
class RoadtaxQrPaymentScreen extends StatefulWidget {
  final Map<String, dynamic> vehicle;
  final Map<String, dynamic> roadTax;
  const RoadtaxQrPaymentScreen({super.key, required this.vehicle, required this.roadTax});

  @override
  State<RoadtaxQrPaymentScreen> createState() => _RoadtaxQrPaymentScreenState();
}

class _RoadtaxQrPaymentScreenState extends State<RoadtaxQrPaymentScreen> with WidgetsBindingObserver {
  late Map<String, dynamic> _roadTax = widget.roadTax;
  late Map<String, dynamic> _payment = (widget.roadTax['payment'] as Map).cast<String, dynamic>();
  ...
  Future<void> _retry() async {
    ...
    try {
      ...
      final taxYear = int.parse(_roadTax['tax_year'].toString());
      final created = await ApiRoadTaxService.create(vehicleId: vehicleId, taxYear: taxYear);
      if (!mounted) return;
      setState(() {
        _roadTax = created;
