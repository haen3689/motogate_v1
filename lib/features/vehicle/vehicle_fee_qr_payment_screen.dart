...
/// Shows the BCEL OnePay QR / one-click-pay deeplink for the vehicle's
/// one-time registration fee, and polls the payment status until BCEL
/// confirms it (or it expires). Mirrors RoadtaxQrPaymentScreen.
class VehicleFeeQrPaymentScreen extends StatefulWidget {
  final Map<String, dynamic> vehicle;
  final Map<String, dynamic> payment;
  const VehicleFeeQrPaymentScreen({super.key, required this.vehicle, required this.payment});

  @override
  State<VehicleFeeQrPaymentScreen> createState() => _VehicleFeeQrPaymentScreenState();
}

class _VehicleFeeQrPaymentScreenState extends State<VehicleFeeQrPaymentScreen> with WidgetsBindingObserver {
  ...
  Future<void> _checkStatus() async {
    ...
    try {
      ...
      if (result['status'] == 'paid') {
        ...
        if (!mounted) return;
        final amount = num.tryParse(result['amount']?.toString() ?? '') ?? 0;
